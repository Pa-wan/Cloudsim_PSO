...
public class Utils {
	 ...
	 * @param vm
	 */
	public static void updateVmResource(Vm vm) {
		vm.setCurrentAllocatedSize(vm.getSize());
		List<Double> list = new ArrayList<Double>();
		list.add(vm.getNumberOfPes() * vm.getMips());
	 ...
	 * @param vm
	 */
	public static void resetVmResource(Vm vm) {
	...
	/*
	 * public static double calcuDistance(Vm vm, Host host) { //向量夹角余弦 double
	 * sum=host.getAvailableMips()*vm.getCurrentRequestedTotalMips()+
	 * host.getRamProvisioner().getAvailableRam()
	 * 
	 * vm.getCurrentRequestedRam()+ host.getStorage()*vm.getSize()+
	 ...
	 * Math.pow(host.getStorage(), 2)+Math.pow(host.getBwProvisioner
	 * 
	 * ().getAvailableBw(), 2));
	 * 
	 * double sum2=Math.sqrt(Math.pow(vm.getCurrentRequestedTotalMips(), 2)+
	 * Math.pow(vm.getCurrentRequestedRam(), 2)+ Math.pow(vm.getSize(),
	 * 2)+Math.pow(vm.getCurrentRequestedBw(), 2));
	 * 
	...
	public static double calcuDistance(Vm vm, Host host) {
		...
		double sum = host.getAvailableMips() * vm.getCurrentRequestedTotalMips() / Math.pow(host.getTotalMips(), 2)
				...
				+ host.getStorage() * vm.getSize()/ Math.pow(Main.storageMap.get(host.getId()), 2) 
		...
		double sum1 = Math.sqrt(Math.pow(host.getAvailableMips() / (host.getTotalMips() + 0.0), 2)
				...
				+ Math.pow(host.getStorage() / (Main.storageMap.get(host.getId()) + 0.0), 2)
		...
		double sum2 = Math.sqrt(Math.pow(vm.getCurrentRequestedTotalMips() /(host.getTotalMips() + 0.0), 2) 
				...
				+ Math.pow(vm.getSize() / (Main.storageMap.get(host.getId()) + 0.0), 2)
	...
	public static double calcuFactor(Vm vm, Host host, double[] hostAvaiable) {
		...
		double sum = hostAvaiable[0] * vm.getCurrentAllocatedMips().get(0)/ Math.pow(host.getTotalMips(), 2) 
				...
				+ hostAvaiable[3] * vm.getCurrentAllocatedSize()/ Math.pow(Main.storageMap.get(host.getId()), 2);
		...
		double sum1 = Math.sqrt(Math.pow(hostAvaiable[0]/ (host.getTotalMips() + 0.0),2)
				...
				+ Math.pow(hostAvaiable[3] /(Main.storageMap.get(host.getId()) + 0.0), 2));
		...
		double sum2 = Math.sqrt(Math.pow(vm.getCurrentAllocatedMips().get(0) /(host.getTotalMips() + 0.0), 2)
				...
				+ Math.pow(vm.getCurrentAllocatedSize()	/ (Main.storageMap.get(host.getId()) + 0.0), 2));
	...
	public static int partion(List<Map.Entry<Host, Double>> list, int left,
			int right) {
		Map.Entry<Host, Double> pivot = list.get(left);
		while (left < right) {
			while (left < right && list.get(right).getValue() <=
			...
			while (left < right && list.get(left).getValue() >=
	...
	public static void getTopKMax(List<Map.Entry<Host, Double>> list,
			int first, int
		...
		if (first < end) {
			...
			if (partionIndex == k - 1)
				...
				getTopKMax(list, first, partionIndex - 1, k);
			else
				getTopKMax(list, partionIndex + 1, end, k);
		}
	}
	 ...
	 * ObjectOutputStream oo = new ObjectOutputStream(bo); oo.writeObject(src);
	 * ByteArrayInputStream bi = new ByteArrayInputStream(bo.toByteArray());
	 * ObjectInputStream oi = new ObjectInputStream(bi); Host
	 * dis=(Host)oi.readObject(); return dis; }
	 * 
	 * public static Vm deepCopy(Vm src) throws Exception{ ByteArrayOutputStream
	 * bo = new ByteArrayOutputStream(); ObjectOutputStream oo = new
	 * ObjectOutputStream(bo); oo.writeObject(src); ByteArrayInputStream bi =
	 * new ByteArrayInputStream(bo.toByteArray()); ObjectInputStream oi = new
	 * ObjectInputStream(bi); Vm dis=(Vm)oi.readObject(); return dis; }
	 */
