 ...
 * 选择资源剩余量最多的物理机作为虚拟机的目标主机[这种选择算法比使用combmff算法要好一些]
 * @author Jungmin Son
 * @since CloudSimSDN 1.0
 */
public class VmAllocationPolicyCombinedLeastFullFirst extends VmAllocationPolicy implements PowerUtilizationMaxHostInterface,ListAllocationPolicy {
	...
	public VmAllocationPolicyCombinedLeastFullFirst(List<? extends Host> list) {
	   ...
	   private void printLogMsg(String msg) {
	        Log.print("CombLFF_Allocator: " + msg + LINE_SEPARATOR);
	    }
	...
	public boolean allocateHostForVm(Vm vm) {
		...
		for(int tries = 0; result == false && tries < numHosts; tries++) {// we still trying until we find a host or until we try all of them
			double moreFree = Double.NEGATIVE_INFINITY;  //负无穷小
			int idx = -1;
